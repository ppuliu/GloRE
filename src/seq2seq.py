...
# Use local version. Caution needed.
# import rnn
# import rnn_cell
...
def basic_rnn_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(scope or "basic_rnn_seq2seq") as scope:
        ...
        # Encoder.
        _, encoder_state = rnn.rnn(cell, encoder_inputs,  sequence_length=en_seq_length,
                                   scope='rnn_encoder', dtype=dtype)
        ...
        return rnn_decoder(decoder_inputs, encoder_state,
                           cell, scope='rnn_decoder')
...
def attention_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(
      scope or "attention_seq2seq", dtype=dtype) as scope:
        dtype = scope.dtype
        # Encoder.
        encoder_outputs, encoder_state = rnn.rnn(cell, encoder_inputs,  sequence_length=en_seq_length,
                                       scope='attention_rnn_encoder', dtype=dtype)
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
    ...
    The seq2seq argument is a function that defines a sequence-to-sequence
    model, e.g.,
    seq2seq = lambda x, y: basic_rnn_seq2seq(x, y, rnn_cell.GRUCell(24))

    Args:
        encoder_inputs: A list of Tensors to feed the encoder; first seq2seq
            input.
        decoder_inputs: A list of Tensors to feed the decoder; second seq2seq
            input.
        targets: A list of 1D batch-sized int32 Tensors (desired output
            sequence).
        ...
        softmax_loss_function: Function (inputs-batch, labels-batch) ->
            loss-batch to be used instead of the standard softmax (the default
            if this is None).
        per_example_loss: Boolean. If set, the returned loss will be a
            batch-sized tensor of losses for each sequence in the batch. If
            unset, it will be a scalar with the averaged loss from all
            examples.
        name: Optional name for this operation, defaults to
            "model_with_buckets".

    Returns:
        A tuple of the form (outputs, losses), where:
            outputs: The outputs for each bucket. Its j'th element consists of
                a list of 2D Tensors. The shape of output tensors can be either
                [batch_size x output_size] or
                [batch_size x num_decoder_symbols]
                depending on the seq2seq model used.
            losses: List of scalar Tensors, representing losses for each
                bucket, or, if per_example_loss is set, a list of 1D
                batch-sized float Tensors.

    Raises:
        ValueError: If length of encoder_inputsut, targets, or weights is
            smaller than the largest (last) bucket.
    """
    if len(encoder_inputs) < buckets[-1][0]:
        raise ValueError("Length of encoder_inputs (%d) must be at least that "
                         "of last bucket (%d)." %
                         (len(encoder_inputs), buckets[-1][0]))
    if len(targets) < buckets[-1][1]:
        raise ValueError("Length of targets (%d) must be at least that of "
                         "last bucket (%d)." %
                         (len(targets), buckets[-1][1]))
    if len(weights) < buckets[-1][1]:
        raise ValueError("Length of weights (%d) must be at least that of "
                         "last bucket (%d)." %
    ...
    with ops.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(
                ...
                losses.append(sequence_loss(outputs[-1],
                                            targets[:bucket[1]],
                                            weights[:bucket[1]]))
